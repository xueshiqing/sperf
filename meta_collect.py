# encoding: utf-8
# A script for meta info collecting.
# date: 2020-11
# by xueshiqing@ACT_LAB

import os
import sys
import time
import requests
import threading
import csv
import subprocess
import re

HADOOP_HOME = "/home/LAB/hadoop-3.0.0-beta1"
RM_HOSTNAME = 'super01'
PORTAL_HOSTNAME = 'super11'
COLOCATED_HOSTNAME = 'super10'
RESULT_PATH = '/home/LAB/xuesq/xuesq/meta_result/'
host_map = {
    'super01': '192.168.7.70',
    'super02': '192.168.7.71',
    'super03': '192.168.7.72',
    'super04': '192.168.7.73',
    'super05': '192.168.7.74',
    'super06': '192.168.7.75',
    'super07': '192.168.7.76',
    'super08': '192.168.7.77',
    'super09': '192.168.7.78',
    'super10': '192.168.7.79',
    'super11': '192.168.7.80',
    'super12': '192.168.7.81',
}


def remote_run(cmd, hostname1=RM_HOSTNAME):
    return os.popen('ssh -T ' + hostname1 + ' ' + cmd).read().splitlines()


def lc_app_status():
    service_num = 4
    cur = 0
    try_num = 0
    while cur < service_num or try_num < 2:
        for host in host_map.values():
            dockers = remote_run('docker ps', host)
            for docker in dockers:
                if 'zxqy-service:v1' in docker:
                    print(host + ': ' 'zxqy-service:v1 has been started')
                    cur += 1
                elif 'zxqy-db:v2' in docker:
                    print(host + ': ' 'zxqy-db:v2 has been started')
                    cur += 1
                elif 'zxqy-portal:v1' in docker:
                    print(host + ': ' 'zxqy-portal:v1 has been started')
                    cur += 1
                elif 'zxqy-zk:v1' in docker:
                    print(host + ': ' 'zxqy-zk:v1 has been started')
                    cur += 1
        print(str(cur) + '/4 cloud service has been started...')
        try_num += 1
        if cur >= service_num:
            return True
    print("error in some services...")
    return False


def submit_word_count(p):
    os.system('/home/LAB/hadoop-3.0.0-beta1/yanshi/word_count_single.sh ' + str(p))


if __name__ == '__main__':
    if lc_app_status():
        pressure = sys.argv[1]
        offline_thread = threading.Thread(target=submit_word_count(p=pressure))
        offline_thread.setDaemon(True)
        offline_thread.start()
        hostname = host_map[PORTAL_HOSTNAME]
        port = 80
        path = RESULT_PATH + str(pressure) + '.csv'
        time.sleep(10)
        os.popen('/home/LAB/xuesq/apache-jmeter-5.1/bin/jmeter -n -t /home/LAB/xuesq/xuesq/jmeter/zxqy.jmx'
                ' -Jhostname=' + str(hostname) + ' -Jport=' + str(port) + ' -l ' + path)
        time.sleep(10)
        file_size = os.path.getsize(path)
        time.sleep(15)
        new_file_size = os.path.getsize(path)
        while new_file_size != file_size:
            file_size = new_file_size
            time.sleep(15)
            new_file_size = os.path.getsize(path)
        print("lc collect finish")
